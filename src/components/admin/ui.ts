export const ui = {
  ...
  button: 'rounded-md bg-destaque px-4 py-2 text-sm font-semibold text-texto hover:bg-destaque-hover disabled:opacity-60',
  ...
  chip: (active: boolean) =>
    `rounded-full px-3 py-1 text-sm ${active ? 'bg-destaque text-texto' : 'bg-superficie-2 text-texto-suave hover:text-texto'}`,
} as const
