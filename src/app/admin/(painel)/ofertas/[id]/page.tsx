...
import { getOfferGroup } from '@/lib/data/offer-groups'
import { OfferGroupForm } from '../offer-group-form'
import { DeleteGroupSection } from '../delete-group-form'
...
export default async function OfertaPage({ params, searchParams }: PageProps<'/admin/ofertas/[id]'>) {
  ...
  const [offer, products] = await Promise.all([
    id !== 'novo' && isUuid(id) ? getOfferGroup(id, store.id) : Promise.resolve(null),
    listProducts(store.id),
  ])
  if (id !== 'novo' && !offer) notFound()
  if (offer && offer.id !== id) redirect(`/admin/ofertas/${offer.id}`)
  const grants = offer?.plans.flatMap(plan => plan.grants) ?? []
  const warnings = offer ? (await Promise.all(products.filter((p) => grants.some(g => g.productId === p.id)).map(async (product) => {
    ...
    const offeredLevels = [...new Set(grants.filter(g => g.productId === product.id).map(g => g.level))]
    const result = assessProductReadiness({ mode: product.contentMode ?? 'sections', modules, offeredLevels })
    return result.emptyLevels.length ? `${product.title}: ${result.emptyLevels.map(level => level === 'basic' ? 'Básico' : 'Completo').join(' e ')} sem material publicado utilizável nesta oferta.` : null
  }))).filter(Boolean) : []
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <OfferGroupForm key={`${offer?.id ?? 'novo'}:${offer?.version ?? 0}:${store.id}`} offer={offer} products={products} initialCode={typeof codigo === 'string' ? codigo : ''} storeId={store.id} />
      {offer && <DeleteGroupSection key={offer.id} offer={{ id: offer.id, name: offer.name }} storeId={store.id} />}
    </div>
  )
}
