...
import * as actions from '@/app/admin/(painel)/ofertas/actions'
...
function form(overrides: Record<string, string> = {}) {
  ...
  for (const [key, value] of Object.entries({ id: offerId, store_id: storeId, confirmation: 'Oferta de teste', ...overrides })) result.set(key, value)
  return result
}
function remove(data = form()) {
  return actions.excluirOferta({ error: null }, data)
}
...
describe('ação de excluir oferta', () => {
  ...
  it('usa loja da sessão na RPC, revalida após exclusão e retorna à lista', async () => {
    await expect(remove()).rejects.toThrow('NEXT_REDIRECT:/admin/ofertas?msg=Oferta excluída. Os produtos foram preservados.')
    expect(io.rpc).toHaveBeenCalledExactlyOnceWith('delete_offer_atomic', { p_id: offerId, p_store_id: storeId, p_confirmation: 'Oferta de teste' })
  ...
  it('apresenta bloqueio de pedido sem relatar sucesso ou invalidar cache', async () => {
    io.rpc.mockResolvedValue({ error: { code: 'P0001', message: 'Oferta possui pedidos. Preserve os acessos.' } })
    expect(await remove()).toEqual({ error: 'Oferta possui pedidos. Preserve os acessos.' })
